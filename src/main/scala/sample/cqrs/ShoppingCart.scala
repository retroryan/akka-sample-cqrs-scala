...
object ShoppingCart {
  ...
  val CART_OPENED = "CART_OPENED"
  val CART_ADJUSTED = "CART_ADJUSTED"
  val CART_REMOVED = "CART_REMOVED"
  val CART_CHECKED_OUT = "CART_CHECKED_OUT"
  ...
  def apply(cartId: String, eventProcessorTags: Set[String]): Behavior[Command] = {
    EventSourcedBehavior
      ...
      .withTagger(event => tagEvent(event,eventProcessorTags))
      .withRetention(RetentionCriteria.snapshotEvery(numberOfEvents = 100, keepNSnapshots = 3))
      .onPersistFailure(SupervisorStrategy.restartWithBackoff(200.millis, 5.seconds, 0.1))
  }

  def tagEvent(event: Event, eventProcessorTags: Set[String]): Set[String] = {
    event match {
      case _: ItemAdded => eventProcessorTags + CART_OPENED
      case _: ItemQuantityAdjusted => eventProcessorTags + CART_ADJUSTED
      case _: ItemRemoved => eventProcessorTags + CART_REMOVED
      case _: CheckedOut => eventProcessorTags + CART_CHECKED_OUT
    }
  }
